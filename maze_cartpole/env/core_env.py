...
class CartPoleCoreEnvironment(CoreEnv):
    ...
    def __init__(
        self, theta_threshold_radians: float, x_threshold: float, reward_aggregator: RewardAggregatorInterface
    ):
        super().__init__()
        ...
        self.env_rng: np.random.RandomState | None = None
        self.seed(None)
        self._setup_env()
    ...
    @override(CoreEnv)
    def step(self, maze_action: CartPoleMazeAction) -> tuple[CartPoleMazeState, np.array, bool, bool, dict[Any, Any]]:
    ...
    def get_maze_state(self) -> CartPoleMazeState:
        ...
        return CartPoleMazeState(
            ...
            pole_angular_velocity=self.pole_velocity,
        )
