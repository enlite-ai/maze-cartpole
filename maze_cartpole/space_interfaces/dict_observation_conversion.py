...
class DictObservationConversion(ObservationConversionInterface):
    ...
    def maze_to_space(self, maze_state: CartPoleMazeState) -> dict[str, np.ndarray]:
        ...
        return {
            'cart_position': np.asarray([maze_state.cart_position], dtype=np.float32),
    ...
    def space(self) -> spaces.Dict:
        ...
        return spaces.Dict(
            {
                'cart_position': spaces.Box(
                    low=np.float32(-self.x_threshold * 2),
                    dtype=np.float32,
                    high=np.float32(self.x_threshold * 2),
                    shape=(1,),
                ),
                'cart_velocity': spaces.Box(
                    low=-np.finfo(np.float32).max, high=np.finfo(np.float32).max, shape=(1,), dtype=np.float32
                ),
                'pole_angle': spaces.Box(
                    low=np.float32(-self.theta_threshold_radians * 2),
                    high=np.float32(self.theta_threshold_radians * 2),
                    shape=(1,),
                    dtype=np.float32,
                ),
                'pole_angular_velocity': spaces.Box(
                    low=-np.finfo(np.float32).max, high=np.finfo(np.float32).max, shape=(1,), dtype=np.float32
                ),
            }
        )
